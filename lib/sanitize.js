Sanitize = {
  ...
  validatePresence: function(detail) {
    ...
    if (! validTask)
      Logger.log('info', 'Listing validation failed because type, source, or url was missing', detail);

    if (! detail.sourceTimestamp)
      Logger.log('info', 'Discarding listing with no timestamp', detail);

    return validBasic && validTask && validTimestamp;
  },

  validateLimits: function(detail) {
    return ((Sanitize.limits.minPrice < detail.price < Sanitize.limits.maxPrice)
      && (Sanitize.limits.minm2 < detail.m2 < Sanitize.limits.maxm2)
      && (Sanitize.limits.minPricem2 < detail.pricem2 < Sanitize.limits.maxPricem2)
